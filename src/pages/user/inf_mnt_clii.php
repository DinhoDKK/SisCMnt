...
if (!empty($_GET['pesquisamat'])) {

    // Buscar as informações do anúncio a ser alterado (no banco de dados)
  $sql = "SELECT * FROM matclii WHERE identidade_usuario = :identidade AND numficha = :pesquisamat";
  try {
    $stmt = $pdo->prepare($sql);

    $stmt->execute(array(':identidade' => $_SESSION['identidade'], ':numficha' => $_GET['pesquisamat']));
    ...
    if ($stmt->rowCount() == 1) {
      ...
      */
      ...
      header("Location: pesquisamat.php?msgErro=Você não tem permissão para acessar esta página");
      die();
    }

  } catch (PDOException $e) {
    header("Location: pesquisamat.php?msgErro=Falha ao obter registro no banco de dados.");
    //die($e->getMessage());
    ...
    <div class="container">
      ...
      <form action="processa_matclii.php" method="post">
        ...
        <div class="col-md-4">
              ...
              value="<?php echo $result['numficha'] ?>">
            </div>
        ...
        <div class="col-md-6">
            ...
            <div class="col-md-4">
              ...
              value="<?php echo $result['dataindisponibilidade'] ?>">
            </div>

            <div class="col-md-10">
              <label for="motivo" class="form-label">Motivo da indisponibilidade</label>
              <textarea class="form-control" id="motivo" name="motivo" rows="5" cols="50"><?php echo $result['motivo'] ?>
            </textarea>
            </div>
